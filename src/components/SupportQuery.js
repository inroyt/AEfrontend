...
import { fetchQueries } from '../redux/user/messageSlice';
import QueryExcerpt from './QueryExcerpt';
const SupportQuery = () => {
  ...
  const queryThread = useSelector((state) => state.messages.supportQueries);
          ...
          {queryThread.map((query, index) => {
            ...
            if (index === queryThread.length - 1) {
              ...
              return (
                ...
                >
                  <QueryExcerpt query={query} page={page}/>
                </div>
              );
            }
