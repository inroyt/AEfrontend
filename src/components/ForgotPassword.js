...
const ForgotPassword = () => {
  ...
  const canReset=!isPasswordError;
...
const startCountdownOTP = () => {
  ...
  const sendOtp = async () => {console.log('sendOtp clicked')
    setIsOtpLoading(true);
    try {
      const response = await api.post(`/api/request-otp-forgot-password`, { phoneNumber: phoneNumber });
  ...
  const verifyOtp = async () => {
    try {
      const response = await api.post('/api/verify-otp-forgot-password', { phoneNumber: phoneNumber, otp: otp });
      if (response.status === 200) {
 ...
 // console.log(verificationResult);
  const resetPassword= async ()=>{
    try{
     const response= await api.post('/api/resetPassword',{newPassword:password,userId:userId});
     if(response.status===200){
    ...
    <div ref={ref} className="flex flex-col items-center min-w-full min-h-screen text-white font-roboto bg-blue-700 dark:bg-slate-600 gap-6 py-20 px-4 overflow-y-auto">
      {!verificationResult ? (
        <div className="w-full lg:w-1/3 h-full flex flex-col mb-20 items-center gap-4 p-2 justify-center border-red-500">
          <p className="text-2xl lg:text-3xl">Reset Password</p>
          {!serverResponse ? (
            <div className="flex flex-col w-full h-full gap-2">
              <input
                className="w-full h-14 lg:h-16 rounded-lg px-2 dark:bg-slate-50 text-gray-700 text-lg outline-none shadow-lg overflow-hidden"
                type="text"
          ...
          <p>
           After OTP verification, you can reset the password.
          </p>
        </div>
      ) : (
        <div className="w-full lg:w-1/3 h-full flex flex-col mb-20 items-center gap-4 p-2 justify-center">
            <p>Your login username is: <span className="font-bold">{verificationResult}</span></p>
          <input 
           type="text"
           className="w-full h-14 lg:h-16 rounded-lg px-2 text-gray-700 text-lg outline-none shadow-lg overflow-hidden" 
