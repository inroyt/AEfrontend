...
import { getSocialPost,setSocialPageState } from './postSlice';
import SocialExcerpt from './SocialExcerpt';
const SocialPostList = () => {
  const page = useSelector((state) => state.post.socialPage);
  const [isLoading, setIsLoading] = useState(false);
  const [isNoPost, setIsNoPost] = useState(false)
  const postContainerRef = useRef(null);
  const lastPostRef = useRef(null);
  const postThread = useSelector((state) => state.post.socialPosts);
  const dispatch = useDispatch();


 
  useEffect(() => {
    dispatch(getSocialPost(page));
  }, [page]);
      ...
      (entries) => {
        if (entries[0].isIntersecting && !isLoading) {
          if (postThread.length >= page * 5) {
            ...
            setTimeout(() => {setIsLoading(false);dispatch(setSocialPageState());}, 1000);
          }
        }
      },
  ...
  return (
    <div className="inset-0 w-full min-h-screen py-16 lg:py-20 overflow-x-clip">
      {postThread.length >= 1 ? (
        <div
