...
import { getPost,setPageState } from './postSlice';
...
const PostList = () => {
  ...
  const postThread = useSelector((state) => state.post.posts);
  ...
  /*
  //set the scroll position to top of the page when the component mounts
  useEffect(() => {
      ...
      (entries) => {
        if (entries[0].isIntersecting && !isLoading) {
          if (postThread.length >= page * 5) {
            ...
            setTimeout(() => {setIsLoading(false);dispatch(setPageState())}, 1000);
          }
        }
      },
  ...
  if(isNoPost){
    return (
      <div className="flex w-screen h-screen gap-4 mx-auto bg-inherit dark:bg-gray-600 rounded-md items-center justify-center mb-2">
            <span >Error Loading Posts,Please Refresh.</span>
          </div>
    )
  }

  return (
    <section className="inset-0 w-screen py-16 lg:py-20">
      {postThread.length >= 1 ? (
        <div
