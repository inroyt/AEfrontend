...
import { deleteApplyList } from './messageSlice';
const ApplyExcerpt = ({query,page}) => {
  ...
  return (
    <div className="flex flex-col flex-grow gap-2 p-4 justify-center dark:bg-slate-700 dark:text-gray-100 rounded-md shadow-md">
      <div className="border-b-2 border-blue-500 py-2 lg:w-2/3" >
        <p className="font-semibold text-base">From: <span className="font-bold text-lg">{query.phoneNumber}</span></p>
      </div>
      <div className="border-b-2 border-blue-500 py-2 lg:w-1/3" >
        <p className="font-semibold text-base">Title: <span className="font-bold text-lg">{query.postName}</span></p>
      </div>
      ...
      <div className="py-2  flex justify-between">
        ...
        <button 
        onClick={()=>dispatch(deleteApplyList(queryId,page))}
