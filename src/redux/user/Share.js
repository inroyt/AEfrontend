...
const Share = ({ post }) => {
  ...
  const url=`https://www.assamemployment.org/post/${postId}`;
...
const handleClick=async ()=> {
  ...
  return (
    <div className="flex items-center justify-evenly mx-4 lg:mx-auto w-[30%] h-full gap-1  cursor-pointer text-gray-600">
      ...
      <div onClick={()=>handleClick()} title="Remove this post" className={`hover:text-blue-500 ${isClicked? `text-blue-500`:``} `}>
      <FaBookmark size="20"/>
      </div>
    </div>
  );
