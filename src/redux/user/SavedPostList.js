...
import { getSavedPost} from '../user/userSlice';
import SavedPostExcerpt from './SavedPostExcerpt';
const SavedPostList = () => {
  ...
  const postThread = useSelector((state) => state.user.savedPosts);
  ...
  useEffect(() => {
    dispatch(getSavedPost(profileId,page));
    ...
    <section className="py-6 mb-4">
      {postThread.length >= 1 ? (
        ...
        <div className="flex w-full gap-4 mx-auto h-full bg-inherit dark:bg-gray-600 rounded-md items-center justify-center mb-2">
          <span >No saved posts have been found</span>
        </div>
       ) : (
