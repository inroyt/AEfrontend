...
import { getOwnPost,clearOwnPosts} from '../user/userSlice';
import OwnPostExcerpt from './OwnPostExcerpt';
const OwnPostList = () => {
  ...
  const [noSavedPost, setNoSavedPost] = useState(false);
  const postContainerRef = useRef(null);
  const lastPostRef = useRef(null);
  const postThread = useSelector((state) => state.user.ownPosts);
  const dispatch = useDispatch();

 useEffect(() =>{ dispatch(clearOwnPosts());},[profileId]);
  useEffect(() => {
 
    if(isOwnProfile){
      dispatch(getOwnPost(profileId,page));
    }else{
      
      dispatch(getOwnPost(otherProfileId,page));
   }
    
  }, [dispatch,profileId,page,isOwnProfile]);
  ...
  }, [isLoading, postThread, page]);
 
  useEffect(() => {
    const timer = setTimeout(() => {
      if (postThread.length === 0) {
        setNoSavedPost(true);
      }
    }, 1500);
  ...
  return (
    <section className="py-6 mb-4 ">
      {postThread.length >= 1 ? (
        ...
        >
          {postThread.map((post, index) => {
            const currentDate = new Date(post.timestamp);
            const prevPost = postThread[index - 1];
            const prevDate = prevPost ? new Date(prevPost.timestamp) : null;

            const showDate =
              !prevDate || currentDate.toDateString() !== prevDate.toDateString();

            if (index === postThread.length - 1) {
              // Last post, attach ref
              return (
                <div
                  ref={lastPostRef}
                  key={index}
                  className="w-full h-full bg-white rounded-lg mb-4 shadow-lg"
                >
                  <OwnPostExcerpt post={post}/>
                </div>
              );
            }

            return (
              <div key={index} className="w-full bg-white h-full rounded-lg mb-4 shadow-lg">
               <OwnPostExcerpt post={post}/>
              </div>
            );
          })}
        ...
        <div className="flex w-full gap-4 mx-auto mt-[15%] h-full bg-inherit items-center justify-center">
          <svg
            aria-hidden="true"
            className="w-8 h-8 text-gray-200 animate-spin dark:text-gray-600 fill-blue-600"
            viewBox="0 0 100 101"
